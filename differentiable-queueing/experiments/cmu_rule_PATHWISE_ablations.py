...
from cmu_rule_PATHWISE import pathwise_cmu
...
def _run_pathwise(kwargs):
    return pathwise_cmu(**kwargs)


def run_sweep(seeds, num_iter, rho, T, queue_class, tag):
    """Run pathwise over all (alpha, gap) pairs in a single pool submission."""
    ...
    with mp.ProcessingPool(NUM_CORES) as pool:
        all_outputs = pool.map(_run_pathwise, all_jobs)
    ...
    out_path = os.path.join(CMU_DIR, f'pathwise_ablation_{tag}.json')
...
def run_ablation_axis(seeds, axis_name, axis_values, cache):
    ...
    for val in axis_values:
        ...
        if cache_key in cache:
            ...
            prev_path = os.path.join(CMU_DIR, f'pathwise_ablation_{prev_tag}.json')
            cur_path = os.path.join(CMU_DIR, f'pathwise_ablation_{tag}.json')
            # symlink or copy the result
