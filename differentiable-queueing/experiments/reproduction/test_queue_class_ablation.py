...
Reference: cmu/pathwise_ablation_queue_class_{5,15,20}.json
           cmu/reinforce_ablation_queue_class_{5,15,20}.json
"""
import os
...
QC_VALUES = [5, 15, 20]  # 10 already done in main reproduction
RHO = 0.95
GAP = 0.5
PW_ALPHA = 0.5
RF_ALPHA = 0.1
...
def run():
    print("=" * 70)
    print("Section 5.2: queue_class ablation reproduction (quick)")
    print("=" * 70)
    ...
    for qc in QC_VALUES:
        qc_key = f"qc_{qc}"
        env_config_base = build_env_config(qc, RHO, GAP)
        pw_all[qc_key] = {str(PW_ALPHA): {}}
        rf_all[qc_key] = {str(RF_ALPHA): {}}
        ...
        pw_all[qc_key][str(PW_ALPHA)][str(GAP)] = trials
        print(f"  PW qc={qc} gap={GAP}: {np.mean(costs):.3f}±{np.std(costs):.3f} ({time.time()-t0:.0f}s)")
        ...
        costs = [r['avg_cost'] for r in trials]
        rf_all[qc_key][str(RF_ALPHA)][str(GAP)] = trials
        print(f"  RF qc={qc} gap={GAP}: {np.mean(costs):.3f}±{np.std(costs):.3f} ({time.time()-t0:.0f}s)")

    with open(os.path.join(RESULTS_DIR, 'queue_class_ablation_pathwise.json'), 'w') as f:
        json.dump(pw_all, f, indent=2)
    with open(os.path.join(RESULTS_DIR, 'queue_class_ablation_reinforce.json'), 'w') as f:
    ...
    for qc in QC_VALUES:
        ...
        for method_name, results_all, ref_pattern, alpha in [
            ('PATHWISE', pw_all, 'pathwise_ablation_queue_class', PW_ALPHA),
            ('REINFORCE', rf_all, 'reinforce_ablation_queue_class', RF_ALPHA),
        ]:
            ref_file = os.path.join(PROJECT_ROOT, 'cmu', f'{ref_pattern}_{qc}.json')
            if not os.path.exists(ref_file):
                print(f"  qc={qc} {method_name}: ref not found")
                continue
            ...
            if ref_gap not in ref[str(alpha)]:
                continue
            oc = [r['avg_cost'] for r in results_all[qc_key][str(alpha)][gap_str]]
            ...
            print(f"  qc={qc:>3} {method_name:>10}: Ours {om:.3f} | Ref {rm:.3f} | {diff:.2f}% [{marker}]")
