#!/bin/bash
# Section 8: Theorem 2 numerical validation
# Submit: grid_run --grid_submit=batch --grid_mem=8G --grid_ncpus=16 ./jobs/reproduction/run_theorem2.sh

eval "$(/user/yc4911/miniconda3/bin/conda shell.bash hook)"
conda activate gpu

cd /user/yc4911/DJ_OR/differentiable-queueing/experiments/reproduction

echo "=== Section 8: Theorem 2 Variance Scaling ==="
echo "Host: $(hostname)"
echo "Date: $(date)"
echo "NSLOTS: ${NSLOTS:-1}"

python test_theorem2_scaling.py

echo "=== Done ==="
echo "Date: $(date)"
